...
*/
void gc_elems(const int nbheads, struct elem *const heads[static nbheads]) {
  /* ajouter votre code ici / add your code here */
  bt1k_reset();
  for (int i = 0; i < nbheads; i++) {
    struct elem const *tete = heads[i];
    while (tete) {
      struct elem const *base = (struct elem const *)memoire_elem_pool;
      assert(tete - base >= 0);
